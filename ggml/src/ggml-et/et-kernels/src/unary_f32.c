...
static inline void vec_sigmoid(float* dst, const float* src, int32_t n) {
    ...
    for (int32_t i = 0; i < n; i += 8) {
        __asm__ volatile(
            ...
            : [r] "=m"(*(float(*)[8])&dst[i])
            : [x] "m"(*(const float(*)[8])&src[i]),
              [z] "m"(zero), [one] "m"(one), [l2e] "m"(log2e)
...
static inline void vec_tanh(float* dst, const float* src, int32_t n) {
    ...
    for (int32_t i = 0; i < n; i += 8) {
        __asm__ volatile(
            ...
            : [r] "=m"(*(float(*)[8])&dst[i])
            : [x] "m"(*(const float(*)[8])&src[i]),
              [one] "m"(one), [two] "m"(two), [tl2e] "m"(two_log2e)
...
static inline void vec_gelu(float* dst, const float* src, int32_t n) {
    ...
    for (int32_t i = 0; i < n; i += 8) {
        __asm__ volatile(
            ...
            : [r] "=m"(*(float(*)[8])&dst[i])
            : [x] "m"(*(const float(*)[8])&src[i]),
              [one] "m"(one), [half] "m"(half), [coef] "m"(coef_a),
...
static inline void vec_gelu_quick(float* dst, const float* src, int32_t n) {
    ...
    for (int32_t i = 0; i < n; i += 8) {
        __asm__ volatile(
            ...
            : [r] "=m"(*(float(*)[8])&dst[i])
            : [x] "m"(*(const float(*)[8])&src[i]),
              [one] "m"(one), [ncl2e] "m"(neg_coef_log2e)
...
static inline void vec_hardsigmoid(float* dst, const float* src, int32_t n) {
    ...
    for (int32_t i = 0; i < n; i += 8) {
        __asm__ volatile(
            ...
            : [r] "=m"(*(float(*)[8])&dst[i])
            : [x] "m"(*(const float(*)[8])&src[i]),
              [z] "m"(zero), [one] "m"(one), [thr] "m"(three), [inv] "m"(inv6)
