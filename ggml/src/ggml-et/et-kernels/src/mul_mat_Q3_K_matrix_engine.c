...
// Q3_K x F32 -> F32 MUL_MAT on the tensor (matrix) engine, TensorFMA32.
// Identical producer/consumer, tiling and tensor-engine loop to
// mul_mat_Q4_K_matrix_engine.c; only the weight dequant differs (Q3_K uses a
// 6-bit quant with a per-16-element int8 scale and no min term).
// Hart 1: dequantize Q3_K weights to FP32 into double-buffered L2 SCP.
...
#define TILE_M  16
#define TILE_N  16
#define BLOCK_K 32      // one Q3_K group (32 elements) per panel
...
//
// A Q3_K super-block packs 256 elements as two 128-element chunks; each 3-bit
// weight is `w = d*(scale-32)*(q2 - (hbit?0:4))` where q2 is 2 low bits from qs
// and hbit is the high bit from hmask; scales are 16 packed 6-bit values.
// kb_group is the global 32-element group index: super-block = kb_group/8,
// group g = kb_group%8, mapping to chunk c = g/4, bit-shift jj = g%4 and hmask
// bit 1<<g (see dequantize_q3_K_block).
static inline void __attribute__((always_inline))
dequant_q3_K_panel(float *panel, const char *src0_batch,
    ...
    const uint8_t hbit  = (uint8_t) (1u << g);
    const int     qoff  = c * 32;
    ...
    for (int j = 0; j < TILE_M; ++j) {
        ...
        for (int l = 0; l < 32; ++l) {
            const int    half = l >> 4;
            const float  dl   = d * (float) (scales[scoff + half] - 32);
            const int8_t low2 = (int8_t)((qs[l] >> shift) & 3);
            const int8_t q    = (int8_t)(low2 - ((hm[l] & hbit) ? 0 : 4));
            panel[l * TILE_M + j] = dl * (float) q;
        }
    }
}
            ...
            for (int64_t unit = my_start; unit < base_units; unit += tiles_stride) {
                ...
                for (int64_t kw = 0; kw < n_windows; ++kw) {
                    ...
                    for (int64_t i = 0; i < kbn; ++i) {
                        dequant_q3_K_panel(cf + i * (SCP_PANEL_SIZE / 4),
