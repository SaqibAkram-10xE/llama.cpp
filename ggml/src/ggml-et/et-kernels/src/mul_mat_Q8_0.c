...
int entry_point(struct ggml_et_binary_params* params, void* env) {
    ...
    // ALWAYS 32 Q8_0 and FP16 supported
    // int block_size;
    // switch (src0->type) {
    //     case GGML_TYPE_Q8_0:
    //         block_size = QK8_0;
    ...
    const int64_t K_blocks = K >> 5;  // Number of quantized blocks per row
    ...
    for (uint64_t base_idx = effective_thread_id * per_thread; 
                    base_idx < total_elements;
                    base_idx += threads_stride) {
        ...
        //     }
            ...
            const int64_t i3 = base_idx / (M * N * ne2);
            const int64_t rem3 = base_idx % (M * N * ne2);
