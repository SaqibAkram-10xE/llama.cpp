...
static inline int keep_element(int32_t tri_type, int64_t i0, int64_t i1) {
    switch (tri_type) {
        case TRI_TYPE_LOWER:      return i0 < i1;
        case TRI_TYPE_LOWER_DIAG: return i0 <= i1;
        case TRI_TYPE_UPPER:      return i0 > i1;
        case TRI_TYPE_UPPER_DIAG: return i0 >= i1;
        default:                  return 0;
    }
}
...
int TRI_F32_FUNC(struct ggml_et_tri_params* params, void* env) {
    ...
    if (ne0 % 16 == 0) {
        ...
        for (int64_t row = thread_id; row < total_rows; row += num_threads) {
            ...
            switch (tri_type) {
                case TRI_TYPE_LOWER:      keep_start = 0;      keep_end = i1;     break;
                case TRI_TYPE_LOWER_DIAG: keep_start = 0;      keep_end = i1 + 1; break;
                case TRI_TYPE_UPPER:      keep_start = i1 + 1; keep_end = ne0;    break;
                case TRI_TYPE_UPPER_DIAG: keep_start = i1;     keep_end = ne0;    break;
                default: return -1;
            }
            if (keep_end > ne0) { keep_end = ne0; }
