...
// Q2_K x F32 -> F32 MUL_MAT on the tensor (matrix) engine, TensorFMA32.
// Identical producer/consumer, tiling and tensor-engine loop to
// mul_mat_Q4_K_matrix_engine.c; only the weight dequant differs (Q2_K uses a
// 6-bit quant with a per-16-element int8 scale and no min term).
// Hart 1: dequantize Q2_K weights to FP32 into double-buffered L2 SCP.
...
#define TILE_M  16
#define TILE_N  16
#define BLOCK_K 32      // one Q2_K group (32 elements) per panel
...
// Dequantize one 32-element Q2_K GROUP of TILE_M weight rows into the FP32 panel,
// written in TenB [k][m] order: panel[k*TILE_M + m].
//
// A Q2_K super-block packs 256 elements as two 128-element chunks; each 2-bit
// weight q is affine `w = d*(sc&0xF)*q - dmin*(sc>>4)` with a 4-bit scale/min
// pair per 16 elements. kb_group is the global 32-element group index:
// super-block = kb_group/8, group g = kb_group%8, mapping to chunk c = g/4 and
// bit-shift index jj = g%4 (see dequantize_q2_K_block).
static inline void __attribute__((always_inline))
dequant_q2_K_panel(float *panel, const char *src0_batch,
    ...
    const int     jj    = g & 3;              // sub-group -> shift 2*jj
    const int     shift = 2 * jj;
    const int     qoff  = c * 32;
    const int     scoff = c * 8 + 2 * jj;

    for (int j = 0; j < TILE_M; ++j) {
        const block_q2_K *blk =
            (const block_q2_K *)(src0_batch + (mb + j) * nb1_0) + sb;
        const float          d   = me_sw_fp16(blk->d);
        const float          dm  = me_sw_fp16(blk->dmin);
        const uint8_t      * qs  = blk->qs + qoff;
        const uint8_t      * scl = blk->scales + scoff;

        for (int l = 0; l < 32; ++l) {
            const int      half = l >> 4;      // 0 -> sub A, 1 -> sub B
            const uint8_t  sc   = scl[half];
            const float    dl   = d  * (float) (sc & 0xF);
            const float    ml   = dm * (float) (sc >> 4);
            const int8_t   q    = (int8_t)((qs[l] >> shift) & 3);
            panel[l * TILE_M + j] = dl * (float) q - ml;
        }
    }
}
            ...
            for (int64_t unit = my_start; unit < base_units; unit += tiles_stride) {
                ...
                for (int64_t kw = 0; kw < n_windows; ++kw) {
                    ...
                    for (int64_t i = 0; i < kbn; ++i) {
                        dequant_q2_K_panel(cf + i * (SCP_PANEL_SIZE / 4),
